...
export async function POST(req: Request) {
  try {
    const { title, code, semesterStart, zipPath } = await req.json();

    if (!title || !semesterStart || !zipPath) {
    ...
    const { data: run, error: rErr } = await supabase
      ...
      .insert({ course_id: course.id, zip_path: zipPath, stage: "extract", status: "queued" })
      .select("id")
      .single();
